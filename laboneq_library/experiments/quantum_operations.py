...
def quantum_gate(qubit, gate_name, pulse_type=pulse_library.drag,
                 additional_pulse_parameters=None):
    pulse_pars = qubit.parameters.drive_parameters_ef if 'ef' in gate_name else \
        qubit.parameters.drive_parameters_ge
    gates = quantum_gates(qubit)
    if gate_name not in gates:
        raise KeyError(f'Gate {gate_name} is not defined. Available gates:\n'
                       f'{list(gates.keys())}.')
    pulse_kwargs = {}
    ...
    return drive_pulse(pulse_type, uid=f'{gate_name} {qubit.uid}',
                       **pulse_kwargs)
