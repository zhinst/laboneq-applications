...
def process_fit_results(
    ...
    Arguments:
        ...
        target_angle:
            ...
            The target_angle is used as initial guess for fitting.
        options:
            The options class for this task as an instance of [DoFittingOption].
...
def plot_population(
    ...
    for q in qubits:
        ...
        if opts.do_fitting:
            ...
            if parameter_to_update is not None:
                ...
                new_parameter_value = qubit_parameters["new_parameter_values"][q.uid][
                    f"{opts.transition}_{parameter_to_update}"
                ]
                # textbox
        ...
        if opts.save_figures:
            workflow.save_artifact(f"Amplitude_Fine_{q.uid}", fig)
