...
options = ResonatorSpectroscopyAnalysisWorkflowOptions
...
def analysis_workflow(
    ...
    with if_(options.do_plotting):
        ...
        with if_(options.do_plotting_magnitude_phase):
            plot_magnitude_phase(
                qubit, processed_data_dict, fit_result, qubit_parameters
            )
        with if_(options.do_plotting_real_imaginary):
...
def plot_magnitude_phase(
    ...
    if opts.save_figures:
        save_artifact(f"Magnitude_Phase_{qubit.uid}", fig)
...
def plot_real_imaginary(
    ...
    if opts.save_figures:
        save_artifact(f"Real_Imaginary_{qubit.uid}", fig)
