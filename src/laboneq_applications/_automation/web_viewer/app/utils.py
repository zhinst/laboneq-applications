...
def build_json_data(
    ...
    graph, layers = automation._get_layered_graph(graph_type)
    pos = hierarchical_layout(graph, layers)

    for idx, key in enumerate(graph):
...
def export_graph_to_json(automation: Automation) -> dict[str, Any]:
    ...
    - nodes: list of node dicts with id, key, status, layer, quantum_elements, x, y
