...
"""This module defines the flux frequency vs flux amplitude experiment.

This is a characterization experiment for CPHASE gate calibration.

In this experiment, we sweep the frequency of a flux pulse on a given coupler
in order to determine a good starting point for the CPHASE pulse.

The experiment has the following pulse sequence:

    q0 --- [ x180_transition ] --- [                            ] --- [ measure ]
    c01--- [                 ] --- [ flux(frequency, amplitude) ] ---
...
from laboneq_applications.contrib.analysis.two_qubit.para_cz_flux_freq_vs_amp_tc import (  # noqa: E501
    analysis_workflow,
)
...
@workflow.workflow(name="para_cz_flux_freq_vs_amp_tc")
def experiment_workflow(
    ...
    *,
    ...
    """The parametric CZ flux frequency vs amplitude calibration workflow.

    Sweeps the frequency and amplitude of a flux pulse on the tunable coupler with
    both qubits prepared in |1>, in order to map the |11>↔|20> chevron and identify
    a starting point (f_opt, A_opt) for a CZ-gate pulse.
    ...
    Arguments:
        ...
        frequencies:
            ...
            (per qubit pair).
        amplitudes:
            Amplitudes of the coupler flux pulse, swept in the inner loop
            (per qubit pair).
...
def create_experiment(
    ...
    """Create the parametric-CZ frequency-vs-amplitude experiment.
    ...
    Arguments:
        ...
        amplitudes:
            Amplitudes of the coupler flux pulse, per qubit pair.
        options:
    ...
    amplitude_sweep_pars = [
        SweepParameter(
            f"amplitude_{e.quantum_element.uid}",
            e_amplitudes,
            axis_name=f"{e.quantum_element.uid} amplitude [a.u.]",
        )
        for e, e_amplitudes in zip(edges, amplitudes, strict=False)
    ]
    ...
    with dsl.acquire_loop_rt(
        ...
        with dsl.sweep(
            ...
            with dsl.sweep(
                name="pulse_amplitude_sweep",
                parameter=amplitude_sweep_pars,
            ):
                ...
                with dsl.section(
                    ...
                    for e, freq, amplitude in zip(
                        edges, frequency_sweep_pars, amplitude_sweep_pars, strict=False
                    ):
                        # excite to |1>
                        qop.x180(e.source_node)
                        qop.x180(e.target_node)
                        # TODO: Do we want phase to be zero at this stage?
                        cz_parameters = {"frequency": freq, "tc_amplitude": amplitude}
                        qop.cz(e.source_node, e.target_node, **cz_parameters)

                    ## measure all qubits
                    with dsl.section(
                        name="main_measure", alignment=SectionAlignment.RIGHT
                    ):
                        for q in qubits:
    ...
    for e in edges:
        calibration[
            e.quantum_element.signals["flux"]
        ].voltage_offset = e.quantum_element.parameters.flux_offset_voltage
        calibration[e.quantum_element.signals["flux"]].range = 1.0
