...
"""This module defines the flux pulse-duration experiment.

This is a characterization experiment for CPHASE gate calibration.

In this experiment, we sweep the duration of a flux pulse on a given coupler in
order to determine a good pulse length for the CPHASE pulse. The frequency and
amplitude are fixed.
...
from laboneq_applications.contrib.analysis.two_qubit.para_cz_flux_pulse_dur_tc import (
    analysis_workflow,
)
...
@workflow.workflow(name="para_cz_flux_pulse_dur_tc")
def experiment_workflow(
    ...
    *,
    ...
    """The parametric CZ pulse-duration calibration workflow.

    Sweeps the duration of a flux pulse on the tunable coupler (at fixed
    frequency and amplitude), with both qubits prepared in |1>, to measure the
    |11>↔|20> oscillation in time. The analysis fits the target population with
    a cosine and extracts the optimal pulse length 1/nu as a 2pi CZ-gate length.
...
def create_experiment(
    ...
    """Create the parametric-CZ pulse-duration experiment.
